...
export default function KarsilastirmaPage() {
  ...
  const startComparison = async () => {
    if (!car1Brand || !car1Model || !car2Brand || !car2Model) {
      setErrorMessage(language === 'tr' ? 'Lütfen iki aracı da seçin.' : 'Please select both vehicles.');
      return;
    }
    ...
    try {
      ...
      if (!data?.car1 || !data?.car2 || !data?.analysis) {
        throw new Error(language === 'tr' ? 'Karşılaştırma sonucu eksik geldi.' : 'Comparison response is incomplete.');
      }

      setResults(data);
    } catch (error) {
      console.error(error);
      setErrorMessage(error instanceof Error ? error.message : t('no_content'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (results && car1Brand && car1Model && car2Brand && car2Model) {
      startComparison();
    }
  }, [language]);

  const canCompare = Boolean(car1Model && car2Model);
  const helperText = errorMessage || (!canCompare ? (language === 'tr' ? 'Karşılaştırma için iki aracı da seçin.' : 'Select both vehicles to start the comparison.') : '');
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <button
              ...
              style={{ padding: '18px 40px', fontSize: '18px', width: '100%', maxWidth: '400px', opacity: (isLoading || !canCompare) ? 0.6 : 1, cursor: (isLoading || !canCompare) ? 'not-allowed' : 'pointer', fontWeight: '800' }}
            >
              {isLoading ? (
            ...
            {helperText && (
              <div
                style={{
                  ...
                  padding: '14px 16px',
                  borderRadius: '14px',
                  background: errorMessage ? 'rgba(231, 76, 60, 0.08)' : 'rgba(255,255,255,0.03)',
                  border: errorMessage ? '1px solid rgba(231, 76, 60, 0.25)' : '1px solid var(--border-color)',
                  color: errorMessage ? '#e74c3c' : 'var(--text-muted)',
                  fontSize: '14px',
                  lineHeight: '1.5',
                  textAlign: 'center',
                }}
        ...
        <div style={{ marginTop: '60px', animation: 'fadeIn 0.6s ease' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px', justifyContent: 'center' }}>
            <div className="card" style={{ flex: '1', minWidth: '320px', padding: '40px', textAlign: 'center', background: 'var(--card-bg)', borderRadius: '24px', border: '1px solid var(--border-color)' }}>
              ...
              <ul style={{ textAlign: 'left', marginTop: '30px', paddingLeft: '0', listStyle: 'none' }}>
                {results.car1.points.map((point, index) => (
                  <li key={index} style={{ marginBottom: '12px', display: 'flex', gap: '10px', fontSize: '15px', lineHeight: '1.4' }}>
                    <span style={{ color: point.includes('+') || !point.includes('-') ? '#2ecc71' : '#e74c3c', fontWeight: 'bold' }}>•</span>
                    {point}
                  </li>
                ))}
              </ul>
            ...
            <div className="card" style={{ flex: '1', minWidth: '320px', padding: '40px', textAlign: 'center', background: 'var(--card-bg)', borderRadius: '24px', border: '1px solid var(--border-color)' }}>
              ...
              <ul style={{ textAlign: 'left', marginTop: '30px', paddingLeft: '0', listStyle: 'none' }}>
                {results.car2.points.map((point, index) => (
                  <li key={index} style={{ marginBottom: '12px', display: 'flex', gap: '10px', fontSize: '15px', lineHeight: '1.4' }}>
                    <span style={{ color: point.includes('+') || !point.includes('-') ? '#2ecc71' : '#e74c3c', fontWeight: 'bold' }}>•</span>
                    {point}
                  </li>
          ...
          <div className="card" style={{ marginTop: '40px', padding: '40px', textAlign: 'left', maxWidth: '1000px', margin: '40px auto 0', border: '2px solid var(--accent-color)', background: 'rgba(252, 163, 17, 0.03)', borderRadius: '28px', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
              ...
              <h3 style={{ fontSize: '24px', fontWeight: '800', margin: 0 }}>{t('ai_vision')}</h3>
            </div>
            <p style={{ fontSize: '17px', lineHeight: '1.8', color: 'var(--text-color)' }}>{results.analysis}</p>
