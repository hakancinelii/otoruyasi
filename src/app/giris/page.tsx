...
export default function LoginPage() {
  ...
  return (
    <main className="container" style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '100px 20px' }}>
      <div className="card" style={{ width: '100%', maxWidth: '450px', padding: '40px', background: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '16px', backdropFilter: 'blur(10px)' }}>
        
        <div style={{ textAlign: 'center', marginBottom: '30px' }}>
          <h1 style={{ fontSize: '32px', margin: '0 0 10px', background: 'linear-gradient(90deg, #fff, var(--accent-color))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Üye Girişi</h1>
          <p style={{ color: 'var(--text-muted)' }}>Oto Rüyası dünyasına geri dönün.</p>
        </div>
        ...
        {isSuccess ? (
          <div style={{ textAlign: 'center', padding: '30px 0' }}>
            ...
            <h2 style={{ fontSize: '20px', color: '#2ecc71', marginBottom: '10px' }}>Giriş Başarılı!</h2>
            <p style={{ color: 'var(--text-muted)' }}>Ana sayfaya yönlendiriliyorsunuz...</p>
          </div>
        ) : (
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '13px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-muted)', cursor: 'pointer' }}>
                <input type="checkbox" style={{ accentColor: 'var(--accent-color)' }} /> Beni Hatırla
              </label>
              <a href="#" style={{ color: 'var(--accent-color)' }}>Şifremi Unuttum</a>
            </div>

            <button type="submit" disabled={loading} className="btn-primary" style={{ width: '100%', padding: '16px', fontSize: '16px', marginTop: '10px', cursor: loading ? 'not-allowed' : 'pointer' }}>
              {loading ? 'Giriş Yapılıyor...' : 'Giriş Yap'}
            </button>

            <div style={{ textAlign: 'center', marginTop: '20px', fontSize: '14px', color: 'var(--text-muted)' }}>
              Hesabınız yok mu? <Link href="/abonelik" style={{ color: 'var(--accent-color)', fontWeight: 600 }}>Hemen Premium'a Geçin</Link>
            </div>
          </form>
        )}
      </div>
