          ...
          <div className="desktop-only" style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <SearchBar /> {user ? (
               <div className="nav-dropdown" onMouseEnter={() => setIsUserOpen(true)} onMouseLeave={() => setIsUserOpen(false)}>
                 ...
                 <div className={`dropdown-menu ${isUserOpen ? 'show' : ''}`} style={{ right: 0 }}>
                    ...
                    <button onClick={logout} className="dropdown-item logout-btn" style={{ background: 'none', border: 'none', width: '100%', textAlign: 'left', cursor: 'pointer', color: '#ff4d4d' }}>🚪 Çıkış</button>
                 </div>
               </div>
            ) : (
            ...
            <div className="lang-switcher" ref={langRef} style={{ position: 'relative' }}>
              <button className="lang-btn" onClick={() => setIsLangOpen(!isLangOpen)}>
                ...
                <span style={{ fontSize: '11px', fontWeight: 700 }}>{String(language).toUpperCase()}</span>
              </button>
              {isLangOpen && (
                <div className="lang-dropdown-menu" style={{ position: 'absolute', top: '100%', right: 0, background: '#1c2128', border: '1px solid var(--border-color)', borderRadius: '10px', overflow: 'hidden', zIndex: 1000, minWidth: '160px', boxShadow: '0 10px 30px rgba(0,0,0,0.5)', marginTop: '8px' }}>
      ...
      <div className={`mobile-sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-content">
           ...
           <div className="sidebar-links">
             ...
             {newsCategories.map((cat) => (
               <Link key={cat.id} href={`/kategori/${cat.id}`} className="sidebar-item secondary" onClick={() => setIsOpen(false)} style={{ opacity: 0.6 }}>{cat.label}</Link>
             ))}
           </div>
        </div>
